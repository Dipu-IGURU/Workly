    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!appId && userId) {
          ...
          const response = await fetch(`${API_BASE_URL}/api/applications?userId=${userId}&sort=desc&limit=1`, { headers });
          const data = await response.json();
        ...
        if (appId) {
          ...
          const headers: HeadersInit = { 'Content-Type': 'application/json' };
          if (token) headers['Authorization'] = `Bearer ${token}`;
          const response = await fetch(`${API_BASE_URL}/api/applications/${appId}`, { headers });
          const data = await response.json();
          ...
          const url = new URL(`${API_BASE_URL}/api/auth/profile/${userId}`);
          if (applicationId) {
